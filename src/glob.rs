...
impl GlobPattern {
                ...
                '{' => {
                    ...
                    if result.is_empty() {
                        for option in options {
                            ...
                            let expanded_option = Self::expand_alternatives(&format!("{}{}{}", prefix, option, suffix));
                        ...
                        for existing in &result {
                            for option in &options {
                                // Recursively expand
                                let expanded_option = Self::expand_alternatives(&format!("{}{}", option, suffix));
    ...
    fn parse_character_class(chars: &[char], start: usize) -> (Component, usize) {
        ...
        (Component::CharacterClass { chars: char_set, negated }, i)
    }
    ...
    pub fn matches(&self, text: &str) -> bool {
        ...
        if !self.expanded_patterns.is_empty() {
            return self.expanded_patterns.iter().any(|pat| {
                GlobPattern::new(pat).matches(text)
            });
        }
    ...
    fn matches_from_position(
        ...
        if text_pos >= text_chars.len() {
            ...
            match &components[component_idx] {
                Component::MultiWildcard => {
                    return self.matches_from_position(text, text_chars, components, component_idx + 1, text_pos);
                },
                _ => {
        ...
        match &components[component_idx] {
            Component::Literal(lit) => {
                ...
                self.matches_from_position(text, text_chars, components, component_idx + 1, text_pos + lit_chars.len())
            },
            Component::SingleWildcard => {
                // ? matches exactly one character, so advance both
                self.matches_from_position(text, text_chars, components, component_idx + 1, text_pos + 1)
            },
            Component::MultiWildcard => {
                // * can match any number of characters (including zero)
                
                // Option 1: * matches nothing, move to next component
                if self.matches_from_position(text, text_chars, components, component_idx + 1, text_pos) {
                    return true;
                }
                
                // Option 2: * matches the current character, try again at next position
                self.matches_from_position(text, text_chars, components, component_idx, text_pos + 1)
            },
